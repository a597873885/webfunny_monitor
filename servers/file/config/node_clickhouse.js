...
const showSql = false  // 控制是否打印 SQL 日志
...
class NodeClickhouse extends WebfunnyClickHouse {
  constructor(schemaPath = "") {
    ...
    this.showSql = showSql  // 保存到实例变量
    
    // 包装 query 方法，添加 OpenTelemetry 追踪（受配置控制）
    wrapClickHouseQuery(this, { serviceName: 'file' })
  }
  ...
  async createTable(options = {}) {
    ...
    if (isCluster) {
      if (this.showSql) console.log(`[NodeClickhouse] 📡 集群模式: 在所有节点创建表 "${tableName}"`)
      
      // 获取集群管理器
      const clusterManager = this.client._clusterManager
      if (!clusterManager) {
        if (this.showSql) console.warn('[NodeClickhouse] ⚠️  集群管理器不存在，使用单节点模式')
      ...
      if (this.schema.Columns && this.schema.Columns.tableName && tableName !== this.schema.Columns.tableName) {
        ...
        if (this.showSql) console.log(`[NodeClickhouse] 📝 表名替换: ${this.schema.Columns.tableName} → ${tableName}`)
      }
      ...
      if (this.schema.Columns && this.schema.Columns.engine) {
        ...
        if (this.showSql) console.log(`[NodeClickhouse] 📌 数据库名: ${databaseName}`)
        ...
        if (this.showSql) console.log(`[NodeClickhouse] 🔧 引擎转换: ${originalEngine} → ${clusterEngine}`)
      }
      
      // 在所有节点上创建表
      const createPromises = clusterManager.nodes.map(async (node) => {
        try {
          if (this.showSql) console.log(`[NodeClickhouse] 📝 在节点 ${node.name} 创建表...`)
          await node.client.command({ query: finalSql })
          if (this.showSql) console.log(`[NodeClickhouse] ✅ 节点 ${node.name} 创建表成功`)
          return { node: node.name, success: true }
        } catch (err) {
          // 表已存在不算错误
          if (err.message && err.message.includes('already exists')) {
            if (this.showSql) console.log(`[NodeClickhouse] ℹ️  节点 ${node.name} 表已存在`)
      ...
      if (failed.length > 0) {
        ...
        if (this.showSql) console.log(`[NodeClickhouse] ✅ 所有节点 (${results.length}) 创建表成功`)
      }
  ...
  async dropTable(options = {}) {
    ...
    if (isCluster) {
      if (this.showSql) console.log(`[NodeClickhouse] 🗑️  集群模式: 从所有节点删除表 "${tableName}"`)
      ...
      const deletePromises = clusterManager.nodes.map(async (node) => {
        try {
          if (this.showSql) console.log(`[NodeClickhouse] 🗑️  从节点 ${node.name} 删除表...`)
          await node.client.command({ 
            query: `DROP TABLE IF EXISTS ${tableName}` 
          })
          if (this.showSql) console.log(`[NodeClickhouse] ✅ 节点 ${node.name} 删除成功`)
          return { node: node.name, success: true }
        } catch (err) {
          // 表不存在不算错误
          if (err.message && err.message.includes('doesn\'t exist')) {
            if (this.showSql) console.log(`[NodeClickhouse] ℹ️  节点 ${node.name} 表不存在`)
      ...
      if (failed.length > 0) {
        ...
        if (this.showSql) console.log(`[NodeClickhouse] ✅ 所有节点 (${results.length}) 删除表 "${tableName}" 成功`)
      }
