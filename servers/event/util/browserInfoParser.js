 ...
 */
function parseBrowserInfo(logInfo, browserInfo) {
  if (!browserInfo) {
    return logInfo
  }

  try {
    // 解析手机品牌 weDeviceBrand
    logInfo.weDeviceBrand = logInfo.weDeviceBrand ? logInfo.weDeviceBrand : getPhoneBrandFromBrowserInfo(browserInfo)

    // 解析设备名称 weDeviceName
    const originDeviceName = logInfo.weDeviceName
    if (!originDeviceName || originDeviceName === "undefined" || originDeviceName === "PC" || originDeviceName.trim() === "") {
      logInfo.weDeviceName = getDeviceNameFromBrowserInfo(browserInfo, originDeviceName)
    }

    // 解析操作系统 os 和 osVersion
    const originOs = logInfo.weOs
    const originOsVersion = logInfo.weOsVersion
    if (!originOs || originOs === "web" || originOs.trim() === "" || !originOsVersion || originOsVersion.trim() === "") {
      const { os, osVersion } = getOsFromBrowserInfo(browserInfo, originOs, originOsVersion)
      logInfo.weOs = os
      logInfo.weOsVersion = osVersion
    }

    // 解析浏览器名称 browserName 和版本 browserVersion
    const originBrowserName = logInfo.weBrowserName
    const originBrowserVersion = logInfo.weBrowserVersion
    if (!originBrowserName || originBrowserName === "其他" || originBrowserName === "undefined" || originBrowserName.trim() === "") {
      const { browserName, browserVersion } = getBrowserInfoFromUA(browserInfo, originBrowserName, originBrowserVersion)
      logInfo.weBrowserName = browserName
      logInfo.weBrowserVersion = browserVersion
    }
  } catch (e) {
    console.log("parseBrowserInfo error:", e)
  }
