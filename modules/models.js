...
class CustomerPvLeaveModel {
   ...
   * @returns {Promise<*>}
   */
  static async createCustomerPvLeaves(logInfoList) {
    if (logInfoList.length === 0) return
    const firstLog = logInfoList[0]
    const proHead = firstLog.webMonitorId
    const dateEnd = firstLog.happenDate.substring(0, 10).replace(/-/g, "") // new Date(firstLog.happenTime).Format("yyyyMMdd")
    const table = proHead + "CustomerPvLeave" + dateEnd

    const keyArray = [`leaveType`, `uploadType`, `happenDate`, `webMonitorId`, `customerKey`,`simpleUrl`,`id`,`createdAt`,`updatedAt`]
    const { keys, valueSql } = CommonSql.handleMultiSql(keyArray, logInfoList)
    let sql = "INSERT INTO " + table + " (" + keys + ") VALUES " + valueSql
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.INSERT })
  }
...
class BehaviorInfoModel {
   ...
   * @returns {Promise<*>}
   */
  static async createBehaviorInfos(logInfoList) {
    if (logInfoList.length === 0) return
    const firstLog = logInfoList[0]
    const proHead = firstLog.webMonitorId
    const dateEnd = firstLog.happenDate.substring(0, 10).replace(/-/g, "") // new Date(firstLog.happenTime).Format("yyyyMMdd")
   ...
   * 获取上一分钟的点击量
   */
  static async getClickCountInRealTimeByMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select count(id) as clickCount from " + CommonSql.setTableName("BehaviorInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' and behaviorType='click'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 获取每一分钟的点击量
   */
  static async getClickCountInRealTimeByEveryMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select DATE_FORMAT(happenDate, '%H:%i') AS time, count(id) as clickCount from " + CommonSql.setTableName("BehaviorInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' and behaviorType='click' group by time"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
...
class CommonModel {
   ...
   * 数据库里所有的表
   */
  static async getAllTableList(dbName) {
    let sql = `
    SELECT 
      table_name 
    FROM
   ...
   * 更新表结构
   */
  static async updateTableFields(stableSqlArr) {
    for (let i = 0; i < stableSqlArr.length; i ++) {
      if (stableSqlArr[i]) {
        await Sequelize.query(stableSqlArr[i], { type: Sequelize.QueryTypes.SELECT})
      }
    }
  }
...
const AlarmRule = Sequelize.import('../schema/alarmRule');
AlarmRule.sync({force: false});
...
class AlarmRuleModel {
   ...
   * @returns {Promise<*>}
   */
  static async createAlarmRule(data) {
    return await AlarmRule.create({
      ...data
    })
  }

  static async deleteAlarmRule(id) {
    await AlarmRule.destroy({
      where: {
        id,
      }
    })
  ...
  static async getAllAlarmRule() {
    let sql = "select * from AlarmRule"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
    /**
   * 获取AlarmRule详情数据
   * @param id  AlarmRule的ID
   * @returns {Promise<Model>}
   */
  static async getAlarmRuleDetail(id) {
    return await AlarmRule.findOne({
      where: {
        id,
      },
    })
  ...
  static async updateAlarmRule(id, data) {
...
class InfoCountByHourModel {
   ...
   * @returns {Promise<Model>}
   */
  static async getPageLoadTimeForAll(webMonitorId, startHour, endHour) {
    let sql = "SELECT uploadType, avg(hourCount) as count from InfoCountByHour where webMonitorId='" + webMonitorId + "' and hourName>='" + startHour + "' and hourName<='" + endHour + "' GROUP BY uploadType"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }

  /**
   * 获取InfoCountByHour每个小时的数据
   * @param id  InfoCountByHour的ID
   * @returns {Promise<Model>}
   */
  static async getAvgLoadTimeForAllByHour(webMonitorId, uploadType, startHour, endHour) {
    let sql = "SELECT hourName, hourCount from InfoCountByHour where webMonitorId='" + webMonitorId + "' and uploadType='" + uploadType + "' and hourName>='" + startHour + "' and hourName<='" + endHour + "'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * @returns {Promise<Model>}
   */
  static async getHttpLoadTimeForAll(webMonitorId) {
    let sql = "SELECT uploadType, avg(hourCount) as count from InfoCountByHour where webMonitorId='" + webMonitorId + "' and hourName>'06-16 00' GROUP BY uploadType"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
  /**
   * 删除InfoCountByHour
...
const LoadTimeInfoByHour = Sequelize.import('../schema/loadTimeInfoByHour');
LoadTimeInfoByHour.sync({force: false});


class LoadTimeInfoByHourModel {
  /**
   * 创建LoadTimeInfoByHour信息
   * @param data
   * @returns {Promise<*>}
   */
  static async createLoadTimeInfoByHour(data) {
    return await LoadTimeInfoByHour.create({
      ...data
    })
  }

  /**
   * 更新LoadTimeInfoByHour数据
   * @param id  用户ID
   * @param status  事项的状态
   * @returns {Promise.<boolean>}
   */
  static async updateLoadTimeInfoByHour(id, data) {
    await LoadTimeInfoByHour.update({
      ...data
    }, {
      where: {
        id
  ...
  /**
   * 获取LoadTimeInfoByHour列表
   * @returns {Promise<*>}
   */
  static async getLoadTimeInfoByHourList() {
    return await LoadTimeInfoByHour.findAndCountAll()
  }

  /**
   * 获取LoadTimeInfoByHour详情数据
   * @param id  LoadTimeInfoByHour的ID
   * @returns {Promise<Model>}
   */
  static async getLoadTimeInfoByHourDetail(id) {
    return await LoadTimeInfoByHour.findOne({
      where: {
        id,
      },
    })
   ...
   * @returns {Promise<Model>}
   */
  static async getLoadTimeInfoByIdByHourName(hourName, webMonitorId, uploadType, deviceName) {
    let sql = "select id from LoadTimeInfoByHour WHERE hourName='" + hourName + "' AND webMonitorId = '" + webMonitorId + "' and uploadType='" + uploadType + "' and showName='" + deviceName + "'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * @returns {Promise<Model>}
   */
  static async getLoadTimeInfoByHourDetailByHourName(hourName, webMonitorId, uploadType) {
    let sql = "select * from LoadTimeInfoByHour WHERE hourName='" + hourName + "' AND webMonitorId = '" + webMonitorId + "' and uploadType='" + uploadType + "'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * @returns {Promise<Model>}
   */
  static async getLoadTimeListByUrl(webMonitorId, startHour, endHour, uploadType) {
    let sql = "SELECT showName, sum(hourCount) as count, avg(hourLoadTime) as loadTime from LoadTimeInfoByHour where webMonitorId='" + webMonitorId + "' and hourName>='" + startHour + "' and hourName<='" + endHour + "' and uploadType='" + uploadType + "' GROUP BY showName order by count desc limit 10"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }

  /**
   * 删除LoadTimeInfoByHour
   * @param id listID
   * @returns {Promise.<boolean>}
   */
  static async deleteLoadTimeInfoByHour(id) {
    await LoadTimeInfoByHour.destroy({
      where: {
        id,
      }
    })
...
class UserModel {
  ...
  static async getUserForAdmin() {
    let sql = "select * from User where userType='admin'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
  ...
  static async getUserListByAdmin() {
    let sql = "select id, userId, userType, nickname, emailName, registerStatus, groupId, createdAt, updatedAt from User"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
  ...
  static async getAllUserInfoForSimple() {
    let sql = "select userId, nickName, emailName from User where registerStatus='1'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
  static async getUsersByUserIds(userIds) {
    ...
    let sql = `select userId, nickName, emailName, phone from User ${userSql}`
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
  /**
   * 激活用户
...
class HttpErrorInfoModel {
  ...
  static async createHttpErrorInfo(data) {
    ...
    let keys = ""
    let values = ""
    const keyArray = [`uploadType`,`happenTime`,`happenDate`,`webMonitorId`,`customerKey`,`simpleUrl`,`completeUrl`,`userId`,`firstUserParam`,`secondUserParam`,`createdAt`,`updatedAt`,`id`,`method`,`httpUrl`,`simpleHttpUrl`,`status`,`statusText`,`statusResult`,`loadTime`]
    keyArray.forEach((key, index) => {
      if (index == keyArray.length - 1) {
        keys += "`" + key + "`"
        let val = data[key]
   ...
   * @returns {Promise<*>}
   */
  static async createHttpErrorInfos(logInfoList) {
    if (logInfoList.length === 0) return
    const firstLog = logInfoList[0]
    const proHead = firstLog.webMonitorId
    const dateEnd = firstLog.happenDate.substring(0, 10).replace(/-/g, "") // new Date(firstLog.happenTime).Format("yyyyMMdd")
  ...
  static async getHttpErrorInfoListByHour(param) {
    ...
    const timeSql = Utils.handleTimeSqlByTimeSize(param)
    let sql = "SELECT distinct(hourName) as hour, hourCount as count from InfoCountByHour where webMonitorId='" + param.webMonitorId + "' and uploadType='" + param.infoType + "' " + timeSql + " order by hourName"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
  /**
   * 获取7天前，24小时内，每小时的错误量
   ...
   * 获取上一分钟http请求量
   */
  static async getHttpErrorCountInRealTimeByMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select count(id) as httpCount from " + CommonSql.setTableName("HttpErrorInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 获取上每分钟http错误请求量
   */
  static async getHttpErrorCountInRealTimeByEveryMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select DATE_FORMAT(happenDate, '%H:%i') AS time, count(id) as httpErrorCount from " + CommonSql.setTableName("HttpErrorInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' group by time"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
...
class ProjectModel {
  ...
  static async getAllProjectList() {
    ...
    let sql = "select id, webMonitorId, userTag, projectName, projectType, recordConfig, createdAt from Project" + whereSql
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
  /**
   * 获取Project列表的警报信息
  ...
  static async getAllProjectListWithAlarm() {
    let sql = "select webMonitorId, projectName, projectType, alarmRuleId, alarmMembers from Project"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
  /**
   * 获取Project个数
  ...
  /**
   * 根据筛选条件获取Project详细信息列表
   * @returns {Promise<*>}
   */
  static async getProjectDetailListBySearch(param) {
    const { appStatus, appType, searchName} = param
    let recordingSql = ""
    if (appStatus === "0") {
      recordingSql = ""
    } else {
      recordingSql = ` and recording='${appStatus}'`
    }
    const appTypeSql = appType === "all" ? "" : ` and projectType='${appType}'`
    const searchNameSql = searchName === "" ? "" : ` and projectName like '%${searchName}%'`
    let sql = `select id, webMonitorId, projectName, projectType, recordConfig, recording, createdAt from Project where 1=1 ` + recordingSql + appTypeSql + searchNameSql
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * @param {} alarmRuleId 
   */
  static async getProjectByAlarmRuleId(alarmRuleId) {
    let sql = "select id from Project where alarmRuleId='" + alarmRuleId + "'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * @returns {Promise<*>}
   */
  static async getProjectConfig(webMonitorId) {
    let sql = "select recordConfig from Project where webMonitorId='" + webMonitorId + "'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 根据webmonitorId 获取项目名
   */
  static async getProjectByWebMonitorIds(webMonitorIds) {
    const webMonitorIdArray = webMonitorIds.split(",")
    let idSql = ""
    webMonitorIdArray.forEach((item, index) => {
      if (index === webMonitorIdArray.length - 1) {
        idSql += `'${item}'`
      } else {
        idSql += `'${item}',`
      }
    })
    let sql = "select * from Project where webMonitorId in (" + idSql + ")"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
  /**
...
class ResourceLoadInfoModel {
   ...
   * 获取上一分钟静态资源错误量
   */
  static async getResourceErrorCountInRealTimeByMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select count(id) as resourceErrorCount from " + CommonSql.setTableName("ResourceLoadInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 获取上每分钟静态资源错误量
   */
  static async getResourceErrorCountInRealTimeByEveryMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select DATE_FORMAT(happenDate, '%H:%i') AS time, count(id) as resourceErrorCount from " + CommonSql.setTableName("ResourceLoadInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' group by time"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
...
class HttpLogInfoModel {
  ...
  static async createHttpLogInfo(data) {
    ...
    let keys = ""
    let values = ""
    const keyArray = [`uploadType`,`happenTime`,`happenDate`,`webMonitorId`,`customerKey`,`simpleUrl`,`completeUrl`,`userId`,`firstUserParam`,`secondUserParam`,`createdAt`,`updatedAt`,`id`,`method`,`httpUrl`,`simpleHttpUrl`,`status`,`statusText`,`statusResult`,`loadTime`,`requestText`,`responseText`]
    keyArray.forEach((key, index) => {
      if (index == keyArray.length - 1) {
        keys += "`" + key + "`"
        let val = data[key]
   ...
   * 批量插入日志
   */
  static async createHttpLogInfos(logInfoList) {
    if (logInfoList.length === 0) return
    const firstLog = logInfoList[0]
    const proHead = firstLog.webMonitorId
    const dateEnd = firstLog.happenDate.substring(0, 10).replace(/-/g, "") // new Date(firstLog.happenTime).Format("yyyyMMdd")
   ...
   * @returns {Promise<*>}
   */
  static async getHttpLogCountByDay(param) {
    const { webMonitorId, timeSize, uploadType } = param
    // let sql = "select count(*) as count from " + CommonSql.setTableName("HttpLogInfo", timeSize, webMonitorId) + " where statusResult='response'"
    // return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})

    let useDay = Utils.addDays(0 - timeSize)
    const startHour = useDay.substring(5) + " 00"
    const endHour = useDay.substring(5) + " 23"
    let sql1 = "SELECT webMonitorId, SUM(hourCount) as count from InfoCountByHour where hourName>='" + startHour + "' and hourName<='" + endHour + "' and webMonitorId='" + webMonitorId + "' and uploadType='" + uploadType + "'"
    ...
    if (timeSize == 0) {
      ...
      let sql2 = "select count(*) as count from " + CommonSql.setTableName("HttpLogInfo", timeSize, webMonitorId) + " where happenDate>='" + startHour + "' and statusResult='response' "
   ...
   * 计算某一小时内接口请求耗时的平均值
   */
  static async calculateHttpLoadTimeByHour(webMonitorId, lastHour, hour) {
    const dateStr = Utils.addDays(-1)
    let tableName = CommonSql.setTableName("HttpLogInfo", 0, webMonitorId)  // Utils.setTableName("HttpErrorInfo")
    if (lastHour === dateStr + " 23:00:00") {
      tableName = CommonSql.setTableName("HttpLogInfo", 1, webMonitorId) 
    }
    let sql = "select avg(loadTime) as loadTime from " + tableName + " WHERE happenDate>='" + lastHour + "' AND happenDate<'" + hour + "' and loadTime>0"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
  /**
   * 计算某一小时内接口请求总数量
   */
  static async calculateHttpTotalCountByHour(webMonitorId, lastHour, hour) {
    const dateStr = Utils.addDays(-1)
    let tableName = CommonSql.setTableName("HttpLogInfo", 0, webMonitorId)  // Utils.setTableName("HttpErrorInfo")
    if (lastHour === dateStr + " 23:00:00") {
      tableName = CommonSql.setTableName("HttpLogInfo", 1, webMonitorId) 
  ...
  /**
   * 根据httpUrl计算某一小时内接口请求数量
   */
  static async calculateHttpCountForUrlByHour(webMonitorId, lastHour, hour) {
    const dateStr = Utils.addDays(-1)
    let tableName = CommonSql.setTableName("HttpLogInfo", 0, webMonitorId)  // Utils.setTableName("HttpErrorInfo")
    if (lastHour === dateStr + " 23:00:00") {
      tableName = CommonSql.setTableName("HttpLogInfo", 1, webMonitorId) 
    }
    let sql = "select simpleHttpUrl as showName, count(simpleHttpUrl) as count, avg(loadTime) as loadTime from " + tableName + " WHERE happenDate>='" + lastHour + "' AND happenDate<'" + hour + "' and statusResult='response' group by simpleHttpUrl order by count desc limit 20"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * @returns {Promise<*>}
   */
  static async getHttpLoadTimePercent(param) {
    const dayStr = Utils.addDays(0 - param.timeSize)
    let sql = "SELECT uploadType, dayCount from InfoCountByDay where webMonitorId='" + param.webMonitorId + "' and uploadType in " + param.uploadTypes + " and dayName='" + dayStr + "'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * @returns {Promise<*>}
   */
  static async getHttpLoadTimeListByUrl(param) {
    const { webMonitorId, timeSize } = param
    let sql = "select simpleHttpUrl, count(simpleHttpUrl) as count, avg(loadTime) as loadTime from " + CommonSql.setTableName("HttpLogInfo", timeSize, webMonitorId) + " where statusResult='response' GROUP BY simpleUrl ORDER BY count desc limit 10"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 获取上一分钟http请求量
   */
  static async getHttpCountInRealTimeByMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select count(id) as httpCount from " + CommonSql.setTableName("HttpLogInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' and statusResult='response'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 获取上每分钟http请求量
   */
  static async getHttpCountInRealTimeByEveryMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select DATE_FORMAT(happenDate, '%H:%i') AS time, count(id) as httpCount from " + CommonSql.setTableName("HttpLogInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' and statusResult='response' group by time"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 获取上一分钟http请求耗时情况
   */
  static async getHttpLoadTimeInRealTimeByMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select IFNULL(avg(loadTime),0) as loadTime from " + CommonSql.setTableName("HttpLogInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' and statusResult='response'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 获取上每分钟http请求耗时情况
   */
  static async getHttpLoadTimeInRealTimeByEveryMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select DATE_FORMAT(happenDate, '%H:%i') AS time, avg(loadTime) as loadTime from " + CommonSql.setTableName("HttpLogInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' and statusResult='response' group by time"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
}
...
class LoadPageInfoModel {
 ...
 /**
   ...
   * 根据httpUrl计算某一小时内接口请求数量
   */
  static async calculateLoadPageInfoForUrlByHour(webMonitorId, lastHour, hour) {
    const dateStr = Utils.addDays(-1)
    ...
    let sql = "select simpleUrl as showName, count(simpleUrl) as count, avg(loadPage) as loadTime from " + tableName + " WHERE happenDate>='" + lastHour + "' AND happenDate<'" + hour + "' group by simpleUrl order by count desc limit 20"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * @returns {Promise<*>}
   */
  static async getPagePerformanceByHour(webMonitorId, lastHour, hour) {
    const dateStr = Utils.addDays(-1)
    let tableName = CommonSql.setTableName("LoadPageInfo", 0, webMonitorId)
    if (lastHour === dateStr + " 23:00:00") {
      tableName = CommonSql.setTableName("LoadPageInfo", 1, webMonitorId)
    }
    ...
    const sql = "SELECT count(id) as firstLoadCount, avg(loadPage) as loadPage, avg(lookupDomain) as dns, avg(connect) as tcp, avg(ttfb) as ttfb, avg(request) as resource, avg(domReady) as domAnalysis  from " + tableName + " WHERE loadType='load' and happenDate>='" + lastHour + "' AND happenDate<'" + hour + "' GROUP BY loadType"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
    // const { searchType, day = 0, webMonitorId, timeHour } = param
    // const start = new Date(parseInt(timeHour, 10)).Format("yyyy-MM-dd hh:00:00")
    // const end = new Date(parseInt(timeHour, 10) + 60 * 60 * 1000).Format("yyyy-MM-dd hh:00:00")
   ...
   * @returns {Promise<*>}
   */
  static async getPageLoadTimeListByUrl(param) {
    const { webMonitorId, timeSize } = param
    let sql = "select simpleUrl, count(simpleUrl) as count, avg(loadPage) as loadPage from " + CommonSql.setTableName("LoadPageInfo", timeSize, webMonitorId) + " GROUP BY simpleUrl ORDER BY count desc limit 10"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * @returns {Promise<*>}
   */
  static async getPageLoadTimePercent(param) {
    const day = Utils.addDays(0 - param.timeSize)
    let sql = "SELECT uploadType, dayCount from InfoCountByDay where webMonitorId='" + param.webMonitorId + "' and uploadType in " + param.uploadTypes + " and dayName='" + day + "'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
}
...
class JavascriptErrorInfoModel {
  ...
  /**
   * 批量创建JavascriptErrorInfo信息
   * @param data
   * @returns {Promise<*>}
   */
  static async createJavascriptErrorInfos(logInfoList) {
    if (logInfoList.length === 0) return
    const firstLog = logInfoList[0]
    const proHead = firstLog.webMonitorId
    const dateEnd = firstLog.happenDate.substring(0, 10).replace(/-/g, "") // new Date(firstLog.happenTime).Format("yyyyMMdd")
  ...
  static async getJavascriptErrorSort(param) {
    const { simpleUrl, timeType, webMonitorId, showType } = param
    ...
    const limitSql = showType === "all" ? "" : " limit 20"
    const sql = "select errorMessage, count(errorMessage) as count from " + tableName + " where infoType='on_error' " + queryStr + " GROUP BY errorMessage order by count desc " + limitSql
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
  /**
   * 获取自定义错误列表
  ...
  static async getConsoleErrorSort(param) {
    ...
    const queryStr = simpleUrl ? " and simpleUrl='" + simpleUrl + "' " : " "
    const limitSql = showType === "all" ? "" : " limit 20"
    const sql = "select errorMessage, count(errorMessage) as count from " + tableName + " where infoType='console_error' " + queryStr + " GROUP BY errorMessage order by count desc " + limitSql
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
  /**
   ...
   * 获取上一分钟JsError的数量
   */
  static async getJsErrorCountInRealTimeByMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select infoType, count(id) as count from " + CommonSql.setTableName("JavascriptErrorInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' group by infoType"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }

  /**
   * 获取上每分钟ConsoleError的数量
   */
  static async getJsConsoleErrorCountInRealTimeByEveryMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select DATE_FORMAT(happenDate, '%H:%i') AS time, count(id) as console_error from " + CommonSql.setTableName("JavascriptErrorInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' AND infoType='console_error' group by time"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 获取上每分钟onError的数量
   */
  static async getJsOnErrorCountInRealTimeByEveryMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select DATE_FORMAT(happenDate, '%H:%i') AS time, count(id) as on_error from " + CommonSql.setTableName("JavascriptErrorInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' AND infoType='on_error' group by time"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 获取上一分钟JsError的UV数量
   */
  static async getJsErrorUVCountInRealTimeByMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select count(distinct customerKey) as uvCount from " + CommonSql.setTableName("JavascriptErrorInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' AND infoType='on_error'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 获取上每分钟JsError的UV数量
   */
  static async getJsErrorUVCountInRealTimeByEveryMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select DATE_FORMAT(happenDate, '%H:%i') AS time,count(distinct customerKey) as jsOnErrorUvCount from " + CommonSql.setTableName("JavascriptErrorInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' AND infoType='on_error' group by time"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 获取上一分钟JsError的UV总数量
   */
  static async getJsUVCountInRealTimeByMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select count(distinct customerKey) as uvCount from " + CommonSql.setTableName("JavascriptErrorInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 获取上每分钟JsError的UV总数量
   */
  static async getJsUVCountInRealTimeByEveryMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select DATE_FORMAT(happenDate, '%H:%i') AS time,count(distinct customerKey) as jsUvCount from " + CommonSql.setTableName("JavascriptErrorInfo", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' group by time"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
...
class CustomerPVModel {
   ...
   * @returns {Promise<*>}
   */
  static async createCustomerPVs(logInfoList) {
    if (logInfoList.length === 0) return
    const firstLog = logInfoList[0]
    const proHead = firstLog.webMonitorId
    const dateEnd = firstLog.happenDate.substring(0, 10).replace(/-/g, "") // new Date(firstLog.happenTime).Format("yyyyMMdd")
  ...
  static async getUVCountForDay(webMonitorId, day, uploadTypeForHour) {
    ...
    const startHour = useDay.substring(5) + " 00"
    const endHour = useDay.substring(5) + " 23"
    let sql1 = "SELECT webMonitorId, SUM(hourCount) as count from InfoCountByHour where hourName>='" + startHour + "' and hourName<='" + endHour + "' and webMonitorId='" + webMonitorId + "' and uploadType='" + uploadTypeForHour + "'"
   ...
   * 每5分钟Uv量
   */
  static async getAliveCusInRealTime(webMonitorId) {
    const nowTime = new Date().getTime()
    const searchTime = new Date(nowTime - 300 * 1000).Format("yyyy-MM-dd hh:mm:ss")
    let sql = "SELECT count(distinct customerKey) as count from " + CommonSql.setTableName("CustomerPV", 0, webMonitorId) + " where happenDate>'" + searchTime + "'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
  ...
  static async getPvCountByMinute(param) {
    ...
    let sql = "SELECT DATE_FORMAT(happenDate,'%Y-%m-%d %H:%i') AS minutes, COUNT(pageKey) AS count FROM " + tableName + " " +
      "WHERE webMonitorId='" + param.webMonitorId + "' and DATE_FORMAT(NOW() - INTERVAL " + param.minuteSize + " MINUTE, '%Y-%m-%d %T') <= happenDate GROUP BY minutes"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
  /**
   * 每分钟的uv量
   ...
   * 获取设备分辨率top10数量列表
   */
  static async getDeviceSizeCountOrderByCount(param) {
    const {webMonitorId, timeSize} = param
    const uploadTypeForDay = UPLOAD_TYPE.SCREEN_COUNT_DAY
    let dayName = Utils.addDays(0 - timeSize)
    let sql1 = "SELECT showName, dayCount as count from DeviceInfoCountByDay where dayName='" + dayName + "' and webMonitorId='" + webMonitorId + "' and uploadType='" + uploadTypeForDay + "' order by count desc limit 10"
    return await Sequelize.query(sql1, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 获取来源网站top10数量列表
   */
  static async getReferrerCountOrderByCount(param) {
    const {webMonitorId, timeSize} = param
    const uploadTypeForDay = UPLOAD_TYPE.REFERRER_COUNT_DAY
    let dayName = Utils.addDays(0 - timeSize)
    let sql1 = "SELECT showName, dayCount as count from DeviceInfoCountByDay where dayName='" + dayName + "' and webMonitorId='" + webMonitorId + "' and uploadType='" + uploadTypeForDay + "' order by count desc limit 10"
    return await Sequelize.query(sql1, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 获取上一分钟的pv、uv量
   */
  static async getPvUvInRealTimeByMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select count(id) as pvCount, count(DISTINCT(customerKey)) as uvCount from " + CommonSql.setTableName("CustomerPV", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
  /**
   * 获取上一分钟的新用户量
   */
  static async getNewUvInRealTimeByMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select count(DISTINCT(customerKey)) as newUvCount from " + CommonSql.setTableName("CustomerPV", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' and newStatus='n_uv'"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 获取每一分钟的pv、uv量
   */
  static async getPvUvInRealTimeByEveryMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select DATE_FORMAT(happenDate, '%H:%i') AS time,count(id) as pvCount, count(DISTINCT(customerKey)) as uvCount from " + CommonSql.setTableName("CustomerPV", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' group by time"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }
   ...
   * 获取每一分钟的新用户量
   */
  static async getNewUvInRealTimeByEveryMinute(webMonitorId, startTimeScope, endTimeScope) {
    let sql = "select DATE_FORMAT(happenDate, '%H:%i') AS time, count(DISTINCT(customerKey)) as newUvCount from " + CommonSql.setTableName("CustomerPV", 0, webMonitorId) + " WHERE happenDate>='" + startTimeScope + "' AND happenDate<'" + endTimeScope + "' and newStatus='n_uv' group by time"
    return await Sequelize.query(sql, { type: Sequelize.QueryTypes.SELECT})
  }

}
module.exports = {CustomerPvLeaveModel,CustomerStayTimeModel,UserTokenModel,BehaviorInfoModel,ScreenShotInfoModel,CommonModel,AlarmRuleModel,ConfigModel,DeviceInfoCountByDayModel,ExtendBehaviorInfoModel,DeviceInfoCountByHourModel,FunnelModel,InfoCountByHourModel,IgnoreErrorModel,InfoCountByDayModel,LoadTimeInfoByHourModel,LocationPointTypeModel,MessageModel,LocationPointModel,LocationPointGroupModel,TeamModel,UserModel,VideosInfoModel,HttpErrorInfoModel,ProjectModel,ResourceLoadInfoModel,HttpLogInfoModel,LoadPageInfoModel,JavascriptErrorInfoModel,CustomerPVModel}