...
const {CustomerPvLeaveModel,CustomerStayTimeModel,ScreenShotInfoModel,ConfigModel,BehaviorInfoModel,DeviceInfoCountByDayModel,CommonModel,DeviceInfoCountByHourModel,ExtendBehaviorInfoModel,FunnelModel,IgnoreErrorModel,InfoCountByHourModel,InfoCountByDayModel,LocationPointTypeModel,LocationPointModel,ProjectModel,UserModel,LocationPointGroupModel,HttpErrorInfoModel,TeamModel,VideosInfoModel,HttpLogInfoModel,LoadPageInfoModel,ResourceLoadInfoModel,JavascriptErrorInfoModel,CustomerPVModel,} = require('../modules/models.js');
    ...
    static async changeJsErrorConfig(ctx) {
        ...
        */
        ...
        await fs.writeFile("./interceptor/config/jsError.js", newString, (err) => {
            ...
            console.log("js错误配置成功，即将重启服务以生效...");
            Common.restartServer()
        });
        ctx.response.status = 200;
        ctx.body = statusCode.SUCCESS_200('success', 0)
    ...
    static async changeConsoleErrorConfig(ctx) {
        ...
        */
        ...
        await fs.writeFile("./interceptor/config/consoleError.js", newString, (err) => {
            ...
            console.log("consoleError错误配置成功，即将重启服务以生效...");
            Common.restartServer()
        });
        ctx.response.status = 200;
        ctx.body = statusCode.SUCCESS_200('success', 0)
    ...
    static async changeHttpErrorConfig(ctx) {
        ...
        */
        ...
        await fs.writeFile("./interceptor/config/httpError.js", newString, (err) => {
            ...
            console.log("httpError错误配置成功，即将重启服务以生效...");
            Common.restartServer()
        });
        ctx.response.status = 200;
        ctx.body = statusCode.SUCCESS_200('success', 0)
    ...
    static async changeResourceErrorConfig(ctx) {
        ...
        */
        ...
        await fs.writeFile("./interceptor/config/resourceError.js", newString, (err) => {
            ...
            console.log("resourceError错误配置成功，即将重启服务以生效...");
            Common.restartServer()
        });
        ctx.response.status = 200;
        ctx.body = statusCode.SUCCESS_200('success', 0)
...
class TeamController {
    ...
    static async createNewTeam(ctx) {
        ...
        let ret = await TeamModel.createTeam(team);
        let data = await TeamModel.getTeamDetail(ret.id);
...
class CommonUtil {
  ...
  static async consoleInfo(startType) {
    ...
    console.log("")
    console.log(new Date().Format("yyyy-MM-dd hh:mm:ss").gray + " ".gray)
    console.log(new Date().Format("yyyy-MM-dd hh:mm:ss").gray + " 作者：一步一个脚印一个坑".white)
    console.log(new Date().Format("yyyy-MM-dd hh:mm:ss").gray + " 微信：webfunny_2020".white)
    console.log(" ")
    console.log(new Date().Format("yyyy-MM-dd hh:mm:ss").gray + " 访问首页地址：", ("http://" + accountInfo.localAssetsDomain + "/webfunny/home.html ").blue.underline)
    console.log(new Date().Format("yyyy-MM-dd hh:mm:ss").gray + " 初始化管理员：", ("http://" + accountInfo.localAssetsDomain + "/webfunny/register.html?type=1 ").blue.underline)
    console.log(new Date().Format("yyyy-MM-dd hh:mm:ss").gray + " 部署问题集合：", "http://www.webfunny.cn/website/faq.html".blue.underline)
    console.log(" ")
    console.log(new Date().Format("yyyy-MM-dd hh:mm:ss").gray + " 启动服务：", "npm run prd".cyan)
    console.log(new Date().Format("yyyy-MM-dd hh:mm:ss").gray + " 重启服务：", "npm run restart".cyan)
    console.log(new Date().Format("yyyy-MM-dd hh:mm:ss").gray + " 停止、删除进程：", "pm2 stop webfunny | pm2 delete webfunny".cyan)
    console.log(" ")
    console.log(new Date().Format("yyyy-MM-dd hh:mm:ss").gray, "研发不易，需要鼓励。去给我们的项目点个 Star 吧".green, "https://github.com/a597873885/webfunny_monitor".cyan.underline)
    console.log(" ")
  }
  ...
  static async checkPurchase(callback, failCallback) {
    ...
    let purchaseCode = purchaseCodeRes[0].configValue
    // 如果配置文件里的激活码有效，则优先使用配置文件里的激活码
    const configPurchaseCodeRes = CommonUtil.decryptPurchaseCode(accountInfo.purchaseCode)
    ...
    await Utils.postJson("http://www.webfunny.cn/config/isBlacklist", {cdkey: purchaseCode, webfunnyVersion}).then((result) => {
    ...
    // 激活码有效性判断
    const { productType, isValid } = CommonUtil.decryptPurchaseCode(purchaseCode)
    if (isValid == true) {
      // 启动试用版激活码，埋点
      if (productType == 0) {
...
let webfunnyVersionForProject = "2.4.7"
class ProjectController {
    ...
    static async createNewProject(ctx) {
        ...
        const { responseTextLength } = accountInfo.httpReqRes
        ...
        // 赞助版和试用版最多只能够创建6个项目
        if (global.monitorInfo.purchaseCodeType <= 3 && projectList.length >= 6) {
            ctx.response.status = 200;
            ctx.body = statusCode.SUCCESS_200('您好，试用(赞助)版用户最多只能够创建 6 个项目，升级为正式版，则可以无限创建项目哦。', 1)
            return
        }
        ...
        if (data) {
            ...
            if (count <= 0) {
                ...
                monitorCode = encodeURIComponent(webfunnyCode.toString()
                                        ...
                                        .replace(/&&&responseTextLength&&&/g, responseTextLength));
...
class UserController {
  ...
  static async login(ctx) {
    ...
    const data = {emailName, password: Utils.md5(decodePwd), registerStatus: 1}
    ...
    if (userData) {
      ...
      UserController.setValidateCode()
      ctx.response.status = 200;
      ctx.body = statusCode.SUCCESS_200('用户名密码不正确，或者未激活，请联系管理员', 1)
    }
  ...
  static async sendRegisterEmail(ctx) {
    ...
    // 1分钟后失效
    setTimeout(() => {
      global.monitorInfo.registerEmailCode[email] = ""
    }, 2 * 60 * 1000)
  ...
  static async register(ctx) {
    const param = Utils.parseQs(ctx.request.url)
    const { name, email, password, emailCode } = param
    const decodePwd = Utils.b64DecodeUnicode(password).split("").reverse().join("")
    const userId = Utils.getUuid()
    const data = {nickname: name, emailName: email, password: Utils.md5(decodePwd), userId, userType: "customer", registerStatus: 0}
...
class CommonUpLog {
    ...
    if (typeof errorLogInfo === "string" && errorLogInfo.indexOf("STAY_TIME") != -1) {
      ...
      try {
        ...
        if (logArray.length === 0) {
          ctx.response.status = 200;
          ctx.body = statusCode.SUCCESS_200("", {d: "d", s: "012345", w: 40})
          return
        }
        // 处理返回值开始
  ...
  static async upMyLog(ctx) {
    ...
    const clientIpString = req.headers['x-forwarded-for'] ||
      ...
      req.socket.remoteAddress ||
      req.connection.socket.remoteAddress;
    const logInfo = JSON.parse(ctx.request.body.data)
  ...
  static async upDLog(ctx) {
    ...
    try {
      ...
      if (cookieInfo) {
        const infoStr = decodeURIComponent(Utils.b64DecodeUnicode(cookieInfo))
...
class Common {
  ...
  static async getWarningList(ctx) {
    const result = global.monitorInfo.warningMessageList
    if (result.length > 3) {
      result.splice(0, result.length - 3)
    }
    ctx.response.status = 200;
    ctx.body = statusCode.SUCCESS_200('success', result)
  }
  ...
  static async getSysInfo(ctx) {
    ...
    // 激活码、消息队列
    // 端口号相关
    const { webfunnyNeedLogin, messageQueue, localServerDomain, localServerPort, localAssetsDomain, localAssetsPort, mainDomain, mysqlConfig } = accountInfo
    ...
    ctx.body = statusCode.SUCCESS_200('success', 
      {
        ...
        localServerDomain, localServerPort, localAssetsDomain, localAssetsPort, mainDomain, adminUserCount
      })
  }
  /**
   * 版本信息
    ...
    switch(minuteTimeStr) {
      ...
      case "01:59":
        TimerCalculateController.handleProject(async(param) => {
          ...
          warningInfo.jsErrorCount = jsErrorData[0].count
          warningInfo.consoleErrorCount = consoleErrorData[0].count
          warningInfo.resourceErrorCount = resourceErrorData[0].count
          warningInfo.httpErrorCount = httpErrorData[0].count
          warningInfo.jsErrorUvCount = jsErrorData[0].uvCount
          warningInfo.consoleErrorUvCount = consoleErrorData[0].uvCount
          warningInfo.resourceErrorUvCount = resourceErrorData[0].uvCount
          warningInfo.httpErrorUvCount = httpErrorData[0].uvCount
          warningInfo.hour = lastHour
...
module.exports = {HttpErrorInfoController,ScreenShotInfoController,CustomerPvLeaveController,CustomerStayTimeController,ConfigController,BehaviorInfoController,FailController,FunnelController,ExtendBehaviorInfoController,IgnoreErrorController,InfoCountByHourController,AlarmController,LocationPointTypeController,LocationPointGroupController,ResourceLoadInfoController,TeamController,LocationPointController,VideosInfoController,CommonUtil,JavascriptErrorInfoController,HttpLogInfoController,LoadPageInfoController,CustomerPVController,ProjectController,UserController,CommonUpLog,Common,TimerCalculateController}