 ...
 * 初始化docker的变量配置
 */
var dockerVariableArray = ['./docker_variable.json']
var dockerVariableConArray = [
  `{
    "purchase": {
      "purchaseCode": "",
      "secretCode": ""
...
dockerVariableArray.forEach((path, index) => {
  fs.readFile(path, "", (err) => {
      if (err) {
          console.log("× " + path + " 配置文件不存在，即将创建...")
          fs.writeFile(path, dockerVariableConArray[index], (err) => {
              if (err) throw err;
              console.log("√ " + path + " 配置文件创建完成！");
          });
      } else {
